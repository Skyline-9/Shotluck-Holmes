...
def separate_shots(obj):
    ...
    if isinstance(obj, dict):
        ...
        audio_captions = obj["audio_captions"]  # Only used in narration captioning
        ...
        for i, shot in enumerate(video_names):
            ...
            shot_obj["conversations"] = [
                {"from": "human",
                 "value": f"<image>\nPlease describe this video. Do not include details that you are not sure of. "
                          f"This is what the speech in the video is saying: {asr[i]}."},
        ...
        shot_obj["conversations"] = [
            {"from": "human",
             "value": f"<image>\nPlease describe this video. Do not include details that you are not sure of. This is "
                      f"what the speech in the video is saying: {whole_asr}."},
...
def main():
    ...
    parser.add_argument(
        "-i", "--input",
        type=str,
        default="data/raw/annotations/20k_train.json",
        help="Path to the input JSON file",
    )
    parser.add_argument(
        "-o", "--output",
        type=str,
        default="data/processed/annotations/20k_train.json",
